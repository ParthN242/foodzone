...
const FoodImage = ({ image, setImage, loadingImg, setLoadingImg }) => {
  const [progress, setProgress] = useState(0);

  const uploadImage = (e) => {
    const imageFile = e.target.files[0];
    firebaseUploadImage(imageFile, setLoadingImg, setProgress, setImage);
  };

  const deleteImgHandler = () => {
    firebaseRemoveImage(image, setImage);
  };
      ...
      {loadingImg ? (
        ...
        <>
          ...
          >
            <svg
              role="status"
              className={`inline w-4 h-4 mr-2 text-gray-200 animate-spin dark:text-orange-500
              `}
              viewBox="0 0 100 101"
              fill="none"
